...
class Process (object):
    ...
    def run(self):
        ...
        if(len(aligned_shape)==68):
            ...
            cv2.rectangle(aligned_face, (aligned_shape[31][0], aligned_shape[29][1]), 
                    (aligned_shape[35][0],aligned_shape[33][1]), (0,255,0), 0)
        else:
            ...
            cv2.rectangle(aligned_face, (aligned_shape[2][0] - 10, aligned_shape[2][1]),  
              (aligned_shape[2][0] + 10, aligned_shape[4][1]),  (0, 255, 0), 2)  
    
        for (x, y) in aligned_shape: 
            cv2.circle(aligned_face, (x, y), 1, (0, 0, 255), -1)

        ROIs = self.fu.ROI_extraction(aligned_face, aligned_shape)
        gray_val = self.sp.extract_color(ROIs)
        ...
        if L == self.buffer_size:
            ...
            idx = np.where((freqs > 20) & (freqs < 50))#the range of frequency that RR is supposed to be within 
            pruned = self.fft[idx]
            pfreq = freqs[idx]
            ...
            idx2 = np.argmax(pruned)#max in the range can be RR
