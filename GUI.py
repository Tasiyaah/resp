...
class GUI(QMainWindow, QThread):
    ...
    def initUI(self):
        ...
        self.lblRR = QLabel(self) #label to show RR change over time
        self.lblRR.setGeometry(900,20,300,40)
        self.lblRR.setFont(font)
        self.lblRR.setText("Frequency: ")
        
        self.lblRR2 = QLabel(self) #label to show stable RR
        self.lblRR2.setGeometry(900,70,300,40)
        self.lblRR2.setFont(font)
        self.lblRR2.setText("Resp rate: ")
        ...
        #self.center()
        self.setWindowTitle("Respiration rate monitor")
        self.show()
    ...
    def main_loop(self):
        ...
        self.lblRR.setText("Freq: " + str(float("{:.2f}".format(self.bpm))))
        
        if self.process.bpms.__len__() >50:
            if(max(self.process.bpms-np.mean(self.process.bpms))<5): #show RR if it is stable -the change is not over 5 bpm- for 3s
                self.lblRR2.setText("Resp rate: " + str(float("{:.2f}".format(np.mean(self.process.bpms)))) + " bpm")
